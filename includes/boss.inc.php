...
if ((include 'includes/bstat.inc.php') !== 1)
{
	error(date(DATE_RFC822),$page,__LINE__,'Error including bstat.inc.php');
}
...
//echo "<div id=hero><p>" . $hname . " attacked for " . $hdam . ", " . $vname ." has ". $vhp . " left <p></div>";
 ...
 if ($nhxp > $htnl or $nhxp == $htnl) {
	...
	echo '<p><a href="index.php?content=boss">Again</a>';
	$nhxp = $nhxp - $htnl;
	$nhtnl = $htnl * 1.2;
	$data = simplexml_load_file('xml/heroes/hero.xml')
		or die(error(date(DATE_RFC822),$page,__LINE__,'Error loading hero.xml for level up'));
	...
	$data->asXML('xml/heroes/hero.xml') 
		or die(error(date(DATE_RFC822),$page,__LINE__,'Error writing to hero.xml for level up'));
	break;
 }
echo '<p><a href="index.php?content=boss">Again</a>';
$data = simplexml_load_file('xml/heroes/hero.xml')
	or die(error(date(DATE_RFC822),$page,__LINE__,'Error loading hero.xml for XP'));
$data->hes->he->xp = $nhxp;
$data->asXML('xml/heroes/hero.xml')
	or die(error(date(DATE_RFC822),$page,__LINE__,'Error writing to hero.xml for XP'));
 break;
 }
if ($hhp > 0) {
if ($vrand == 5) {
